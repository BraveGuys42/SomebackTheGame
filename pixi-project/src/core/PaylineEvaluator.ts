...
export class PaylineEvaluator {
  ...
  constructor(paylines: PaylineData[], symbols: SymbolData[]) {
  ...
  evaluate(grid: GridView): EvaluationResult {
    ...
    // build win grid
    const winGridMask = this.#buildWinGrid(
      paylines,
      grid.length,
      grid[0].length,
    );
  ...
  #buildScoreMap(data: SymbolData[]): Map<string, number[]> {
    return new Map(data.map((item) => [item.Symbol, item.Score]));
  }
}
